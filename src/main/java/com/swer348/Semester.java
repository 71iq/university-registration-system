 ...
 * It handles the creation and management of various components like students,
 * courses, faculty, rooms, sections, and lectures.
 * The class includes methods for creating a new semester, assigning students to
 * courses, assigning instructors, generating lectures,
...
public class Semester {
    ...
    private static void assignInstructors() {
        ...
        for (Section section : sections) {
            ...
            if (instructor.getCredits() + section.getCredits() >= 18)
                continue;
        ...
        private void generateLecturesForSection(Section section) {
            synchronized (lectures) {
                if (section.getStudentsNumber() > 0)
                    for (int i = 0; i < section.getNumberOfLectures(); i++) {
    ...
    private static void assignRoomToLecture() {
        ...
        List<Integer> hours = new ArrayList<>(IntStream.range(8, 17).boxed().toList()),
                minutes = new ArrayList<>(IntStream.range(0, 6).map(e -> e * 10).boxed().toList());
        // may seem slow but time complexity worst case (very rare) almost = 270 * 5 * 9
        // * 6 * 60 * 7(if statement conditions) * 5(isNotBusy method)= 153 million loop
